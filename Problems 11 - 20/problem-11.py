	...
	for y in range(height):
		for x in range(width):
			if x + CONSECUTIVE <= width:
				solution = max(grid_product(x, y,  1, 0, CONSECUTIVE), solution)
			if y + CONSECUTIVE <= height:
				solution = max(grid_product(x, y,  0, 1, CONSECUTIVE), solution)
			if x + CONSECUTIVE <= width and y + CONSECUTIVE <= height:
				solution = max(grid_product(x, y,  1, 1, CONSECUTIVE), solution)
...
# I'd imagine there's probably a more efficient way of solving this...
GRID = [
